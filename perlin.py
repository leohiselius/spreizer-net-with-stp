...
def generate_perlin(N, perlin_scale, save=False, PATH='', input_seed=0):
    size = N//perlin_scale
    assert(size > 0)
    nrow = N

    x = y = np.linspace(0, size, nrow)
    n = [[noise.pnoise2(i, j, repeatx=size, repeaty=size, base=input_seed)
    ...
    landscape = np.reshape(landscape, (N, N))
    if save:
        ...
        plt.savefig(PATH+'_perlinnoise')
        plt.close()

        x = np.reshape(landscape, (int(N**2)))
        plt.hist(x, density=False, bins=32)
        plt.xlabel('angle')
        plt.ylabel('density')
        plt.savefig(PATH+'_perlinhistogram')
        plt.close()
